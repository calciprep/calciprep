...
export default function SSCCHSL_LiveTakePage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center max-w-md shadow-sm">
          ...
          <button onClick={() => router.push('/live-tests/typing/ssc_chsl')} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-bold">
            Back to Overview
          </button>
        </div>
      </div>
  ...
  const examRules = {
    ...
    duration: 10 * 60, // 10 minutes (600 seconds) for CHSL
    targetWpm: 35,
    allowBackspace: true,
    showPassage: true,
  };
  ...
  const handleFinish = (stats: TypingResultType) => {
    ...
    router.push('/live-tests/typing/ssc_chsl/result');
  };
              ...
              >
                <div>
                  ...
                  <p className="text-slate-500 text-xs font-medium leading-relaxed">
                    Official layout replicating the real SSC CHSL Tier-II examination center.
                  </p>
                </div>
                <span className="mt-4 text-[11px] font-bold text-indigo-600 uppercase tracking-wider">TCS Standard</span>
              </div>
