...
export default function CHSLResult({
  result,
  onRestart,
  onTakeAnother,
  isHistoryView = false,
  ...
  const sscStats = useMemo(() => {
    ...
    const isQualified = errorPercentage <= 7.00;
  ...
  const headerTitle = result.testName?.replace('Typing Test - ', '') || 'SSC CHSL Typing Result';
  ...
  return (
    <div className="typing-result-wrapper" style={{ paddingTop: '120px' }}>
      ...
      <div className="stats-grid">
        ...
        <StatCard icon="/media/result-icons/qualification-resul.svg" label="Qualification" value={sscStats.isQualified ? 'Passed' : 'Failed'} description="Error Percentage ≤ 7%" isGradient={true} gradientClass={sscStats.isQualified ? 'qualification' : ''} />
        <StatCard icon="/media/result-icons/marks-result.svg" label="Evaluation Type" value="Qualifying" description="CHSL typing is qualifying in nature" isGradient={true} gradientClass="marks" />
      </div>
      ...
      <div className="formula-section">
        <button className="formula-toggle-btn" onClick={() => setShowFormulas(!showFormulas)}>
          <div className="formula-title-left"><Calculator size={24} color="#3b82f6" /><span className="formula-title-text">SSC CHSL Official Formulas</span></div>
