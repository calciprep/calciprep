...
const FeaturesSection = () => {
  ...
  useEffect(() => {
    ...
    // FIX: Changed let to const as these controllers are not reassigned
    let cardStreamControllerInstance: CardStreamController;
    let particleSystemInstance: ParticleSystem;
    let particleScannerInstance: ParticleScanner; // Still declared with let initially
    ...
    class CardStreamController {
        ...
        init() { this.populateCardLine(); this.calculateDimensions(); ScrollTrigger.create({ trigger: "#features", start: "top bottom", end: "bottom top", onEnter: ()=>this.startAnimation(), onLeave: ()=>this.stopAnimation(), onEnterBack: ()=>this.startAnimation(), onLeaveBack: ()=>this.stopAnimation() }); this.animate = this.animate.bind(this); /*Removed duplicate animate call */ const boundCalc = this.calculateDimensions.bind(this); window.addEventListener("resize", boundCalc); cleanupFuncs.push(() => window.removeEventListener("resize", boundCalc)); }
        startAnimation() { this.isAnimating = true; this.lastTime = performance.now(); if (particleScannerInstance) particleScannerInstance.isAnimating = true; } // Use instance variable
        stopAnimation() { this.isAnimating = false; if (particleScannerInstance) particleScannerInstance.isAnimating = false; } // Use instance variable
        calculateDimensions() { if (!this.cardLine.children[0]) return; this.cardLineWidth = (this.cardLine.children[0].getBoundingClientRect().width + 40) * this.cardLine.children.length; }
        animate() { if (this.isAnimating) { const dt = (performance.now() - this.lastTime) / 1000; this.lastTime = performance.now(); this.position += this.velocity * this.direction * dt; this.updateCardPosition(); } animationFrameId.current = requestAnimationFrame(this.animate); } // Keep RAF outside conditional
        ...
        updateCardClipping() {
            ...
            this.cardLine.querySelectorAll<HTMLElement>(".beam-card-wrapper").forEach((w) => {
                ...
                if (r.left < scannerX && r.right > scannerX) {
                    anyActive = true;
                    const clip = ((scannerX - r.left) / r.width) * 100;
                    nc.style.setProperty("--clip-right", `${clip}%`);
                    ac.style.setProperty("--clip-left", `${clip}%`);
                }
                else if (r.right < scannerX) {
            ...
            if (particleScannerInstance) particleScannerInstance.setScanningActive(anyActive); // Use instance variable
        }
    }
    class ParticleScanner {
        ...
        constructor() { this.canvas = scannerCanvas; const context = this.canvas.getContext("2d"); if(!context) throw new Error('Could not get 2d context'); this.ctx = context; this.w = window.innerWidth; this.h = 300; this.lightBarX = this.w / 2; this.scanningActive = false; this.isAnimating = false; this.glow = 1; this.speed = 0.05; this.setupCanvas(); this.animate = this.animate.bind(this); /* removed duplicate animate call */ const boundResize = this.onResize.bind(this); window.addEventListener("resize", boundResize); cleanupFuncs.push(() => window.removeEventListener("resize", boundResize)); }
        ...
        setScanningActive(a: boolean) { this.scanningActive = a; }
        animate() { if (this.isAnimating) this.render(); animationFrameId.current = requestAnimationFrame(this.animate); } // Keep RAF outside conditional
        render() {
    ...
    class ParticleSystem {
        ...
        constructor() {
            ...
            // Ensure featuresSectionRef.current exists before accessing offset properties
            const initialWidth = featuresSectionRef.current?.offsetWidth || window.innerWidth;
            const initialHeight = featuresSectionRef.current?.offsetHeight || window.innerHeight;
        ...
        animate() {
            animationFrameId.current = requestAnimationFrame(this.animate); // Keep RAF outside conditional
            if (!this.particles || !this.renderer || !featuresSectionRef.current) return;
            const pos = this.particles.geometry.attributes.position.array as Float32Array;
            const sectionWidth = featuresSectionRef.current.offsetWidth; // Cache width
            for (let i = 0; i < this.pCount; i++) {
                pos[i * 3] += this.vel[i];
                // Use cached width
                if (pos[i * 3] > sectionWidth / 2 + 50) {
                    pos[i * 3] = -sectionWidth / 2 - 50;
                }
            }
    ...
    // FIX: Changed let to const here where instances are assigned
    cardStreamControllerInstance = new CardStreamController();
    particleSystemInstance = new ParticleSystem();
    particleScannerInstance = new ParticleScanner();
    // Start animations after instances are created
  ...
  }, []); // Keep dependencies array empty if setup should only run once
