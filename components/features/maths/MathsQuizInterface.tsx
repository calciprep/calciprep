...
const MathsQuizInterface = () => {
    ...
    const finishGame = useCallback(() => {
        ...
        if (!historySavedRef.current) {
            ...
            if (uid) {
                const finishedQuestions = questionsRef.current;
                const totalQuestions = finishedQuestions.length;
                const correctAnswers = finishedQuestions.filter(q => q.isCorrect).length;
                const accuracy = totalQuestions > 0 ? (correctAnswers / totalQuestions) * 100 : 0;

                UserService.addHistory(uid, 'maths_history', {
                    category: `${difficulty} · ${types.join(', ')}`,
                    name: getModeTitle(mode),
                    score: Number(accuracy.toFixed(1)),
                    accuracy: Number(accuracy.toFixed(1)),
                }).catch((error) => {
                    console.error('Failed to save maths history:', error);
                });
            }
        }
    // FIX: Added gameStatus to dependency array as it's used inside
    }, [gameStatus, difficulty, types, mode]);
