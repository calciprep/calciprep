...
const Header = () => {
  const { currentUser, openModal, logout, showNotification } = useAuth();
      ...
      >
        <div className={`max-w-7xl mx-auto bg-white/40 backdrop-blur-xl border border-white/20 shadow-lg rounded-full`}>
          <div className="flex justify-between items-center h-14 px-4 sm:px-6 py-2">
              ...
              <nav className="hidden md:flex items-center space-x-6 lg:space-x-10 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                  ...
                  {currentUser ? (
                    <Link href="/account" className="text-gray-700 hover:text-gray-900 transition-colors text-base font-medium">Dashboard</Link>
                  ) : (
                    <button
                      type="button"
                      onClick={() => {
